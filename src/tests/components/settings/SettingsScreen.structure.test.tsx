...
describe("SettingsScreen structure", () => {
  ...
  it("keeps the approved hierarchy and native window controls", () => {
    ...
    expect(main).toHaveClass("settings-page", "settings-page--dark");
    expect(main.querySelector(".settings-page__body")).toBeInTheDocument();
    ...
    expect(screen.getByRole("banner").querySelector(".settings-page__heading"))
  ...
  it("tracks the color mode on the settings root while retaining legacy classes", () => {
    ...
    expect(main).toHaveClass("settings-page--light");
  });
});
